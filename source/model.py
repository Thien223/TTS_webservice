...
from source.layers import ConvNorm, LinearNorm
from source.utils import to_gpu, get_mask_from_lengths
...
#         mask: Mask for padded data if training, expects None for inference
#         """
#         decoder_state = DecoderState(memory, mask,self.attention_rnn_dim, self.decoder_rnn_dim,self.encoder_embedding_dim, self.attention_layer)
...
#
#         decoder_state.attention_hidden, decoder_state.attention_cell = self.attention_rnn(cell_input, (decoder_state.attention_hidden, decoder_state.attention_cell))
#
#         decoder_state.attention_hidden = F.dropout(decoder_state.attention_hidden, self.p_attention_dropout, self.training)
#
#         attention_weights_cat = torch.cat(
#             (decoder_state.attention_weights.unsqueeze(1),
#              decoder_state.attention_weights_cum.unsqueeze(1)), dim=1)
#         decoder_state.attention_context, decoder_state.attention_weights = decoder_state.attention_layer(
...
#         decoder_state.decoder_hidden, decoder_state.decoder_cell = decoder_state.decoder_rnn(
#             decoder_input, (decoder_state.decoder_hidden, decoder_state.decoder_cell))
#         decoder_state.decoder_hidden = F.dropout(
#             decoder_state.decoder_hidden, decoder_state.p_decoder_dropout, decoder_state.training)
#
#         decoder_hidden_attention_context = torch.cat(
#             (decoder_state.decoder_hidden, decoder_state.attention_context), dim=1)
#         decoder_output = decoder_state.linear_projection(
#             decoder_hidden_attention_context)
#
#         gate_prediction = decoder_state.gate_layer(decoder_hidden_attention_context)
